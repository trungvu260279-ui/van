...
import './DeadlineIntro.css';

const DeadlineIntro = ({ onStart }) => {
    // Tổng thời gian intro (giây)
    const TOTAL_TIME = 7; 
    
    const [timeLeft, setTimeLeft] = useState(TOTAL_TIME);
    const [isFinished, setIsFinished] = useState(false);
    const [writeSpeed, setWriteSpeed] = useState('1s');
    ...
    useEffect(() => {
        ...
        const panicTimer1 = setTimeout(() => setWriteSpeed('0.5s'), 2000);
        const panicTimer2 = setTimeout(() => setWriteSpeed('0.1s'), 4000);

        return () => {
            clearInterval(interval);
            clearTimeout(panicTimer1);
            clearTimeout(panicTimer2);
        };
    }, []);
    ...
    return (
        <div className="deadline-overlay">
            <div className="deadline-container">
                ...
                >
                    ...
                    <g>
                        ...
                        <rect 
                            ...
                            fill="#BE002A" 
                            width="586" height="103" 
                            style={{ animationDuration: `${TOTAL_TIME}s` }}
                        />
                    </g>

                    <g id="death-group" style={{ animationDuration: `${TOTAL_TIME}s` }}>
                    ...
                    <g id="designer-arm-grop" style={{ animationDuration: writeSpeed }}>
                ...
                <div 
                    className={`deadline-text-box ${isFinished ? 'start-btn-active' : ''}`}
                    onClick={handleStartClick}
                >
                    {isFinished ? (
                        <div className="mask-red" style={{ width: '100%', background: 'transparent' }}>
                            <div className="inner" style={{fontSize: '4rem', fontWeight: 'bold', letterSpacing: '5px'}}>BẮT ĐẦU</div>
                        </div>
                    ) : (
                        <>
                            <div className="mask-red" style={{ animationDuration: `${TOTAL_TIME}s` }}>
                                {/* TĂNG CỠ CHỮ LÊN 4rem */}
                                <div className="inner" style={{fontSize: '4rem', fontWeight: 'bold', letterSpacing: '2px'}}>
                                    {timeLeft}s
                                </div>
                            </div>
